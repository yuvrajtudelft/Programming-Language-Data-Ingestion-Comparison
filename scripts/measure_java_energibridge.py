#!/usr/bin/env python3
"""
Run the Java benchmark multiple times with EnergiBridge and generate a report.
"""
...
JAVA_JAR = JAVA_DIR / "target" / "ingestion-java-1.0.0.jar"
...
RUNS_DIR = REPO_ROOT / "data" / "out" / "energibridge_runs_java"
REPORT_CSV = REPO_ROOT / "data" / "out" / "energibridge_java_report.csv"
REPORT_MD = REPO_ROOT / "data" / "out" / "energibridge_java_report.md"
...
def build_java_jar() -> None:
    ...
    if proc.returncode != 0:
        print(proc.stdout)
        print(proc.stderr)
        raise SystemExit(f"Java build failed: {proc.returncode}")
...
def write_report(results: list[dict[str, float | int | str]]) -> None:
    ...
    lines = [
        "# EnergiBridge Java Measurement Report",
        "",
